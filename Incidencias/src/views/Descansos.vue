                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="input-group input-group-sm mb-3">
                            ...
                            <input type="text" class="form-control" aria-label="Small" v-model="id" aria-describedby="inputGroup-sizing-sm">
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Usuario</span>
                            </div>
                            <input type="text" class="form-control" aria-label="Small" v-model="usuario" aria-describedby="inputGroup-sizing-sm">
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Fecha-Inicio</span>
                            </div>
                            <input type="text" class="form-control" aria-label="Small" v-model="fechainicio" aria-describedby="inputGroup-sizing-sm">
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Fecha-descanso</span>
                            </div>
                            <input type="text" class="form-control" aria-label="Small" v-model="fechafinal" aria-describedby="inputGroup-sizing-sm">
                        </div>
                        <div class="input-group input-group-sm mb-3">
                            <div class="input-group-prepend">
                                <span class="input-group-text" id="inputGroup-sizing-sm">Tipo de descanso</span>
                            </div>
                            <input type="text" class="form-control" aria-label="Small" v-model="tipoVa" aria-describedby="inputGroup-sizing-sm">
                        </div>
                        <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" @click="Agregardescanso()">Añadir</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="header">
            <div class="nav-inner">
                <div class="logo">
                    ...
                    <a style="width: 4%; height: 100%; position: absolute;" href="/home"></a>
                    <h2>Nisira</h2>  
                </div>
            </div>
            ...
            <table class="table">
                ...
                <tbody>
                    <tr v-for="vip in vips">
                        <th scope="row">{{ vip.fk_TG20VacAct_TG30Usu }}</th>
                        <th scope="row">{{ vip.cnombre }}</th>
                        <th scope="row">{{ formatearfecha(vip.dFechaInicio) }}</th>     
                        <th scope="row">{{ formatearfecha(vip.dFechaFin) }}</th>
                        <th scope="row">{{ vip.cTipo }}</th>
                    </tr>
                </tbody>
            </table>
            <div class="footer-contenedor">
                <p>Si trabajaste domingo, no podras descansar el Lunes</p>
                <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal" @click="openModal">Añadir</button>
            </div>
        </div>
    </div>
</template>
...
    }, methods: {
        ...
        async traervips(){
            const data = await axios.get('http://localhost:8089/api/v1/vacaciones')
            this.vips = data.data
